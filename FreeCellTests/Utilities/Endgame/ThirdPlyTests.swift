...
private struct ThirdPlyTests {
    let subject: EndgameStateMachine.ThirdPly!
    let helper = MockEndgameHelper()
    var inputLayout = Layout()

    init() {
        inputLayout.columns[0].cards = [Card(rank: .two, suit: .clubs)]
        subject = EndgameStateMachine.ThirdPly(
            index: 4,
    ...
    @Test("nextState: if helper not a win, returns autoplay3, input layouts plus helper layout, backtracking to ThirdPly next index")
    func helperNoWin() throws {
        var layout = Layout()
        layout.columns[0].cards = [Card(rank: .three, suit: .clubs)]
        helper.layoutToReturn = layout
        let result = subject.nextState()
        let auto = try #require(result as? EndgameStateMachine.Autoplay3)
        #expect(auto.accumulatedLayouts == [Layout(), Layout(), inputLayout, layout])
        let backtrack = try #require(auto.backtrackState as? EndgameStateMachine.ThirdPly)
    ...
    @Test("nextState: if helper returns identical layout to input, returns ThirdPly with next index")
