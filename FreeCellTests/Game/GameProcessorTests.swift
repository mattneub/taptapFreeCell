...
struct GameProcessorTests {
    ...
    func deal() async {
        subject.state.firstTap = Tap(category: .column, index: 0)
        ...
        #expect(subject.state.firstTap == nil)
    ...
    func autoplay() async {
        ...
        layout.freeCells[0].card = .init(rank: .two, suit: .hearts)
        layout.freeCells[1].card = .init(rank: .ace, suit: .hearts)
    ...
    @Test("tap: if firstTap is nil, becomes firstTap if not empty source, not foundation")
    func tapFirst() async {
        do {
        ...
        do {
            ...
            await subject.receive(.tapped(.init(category: .foundation(.spades), index: 0)))
            #expect(subject.state.firstTap == nil)
            #expect(presenter.statesPresented.isEmpty)
        }
        do {
            ...
            await subject.receive(.tapped(.init(category: .column, index: 0)))
            #expect(subject.state.firstTap == Tap(category: .column, index: 0))
            #expect(presenter.statesPresented == [])
        }
    }

    @Test("tap: if firstTap is nil, if corresponding card can be autoplayed, it is")
    func tapFirstCanAutoplay() async {
    ...
    @Test("tap: if firstTap exists, if this tap is any foundation, moves firstTap card if movable")
    ...
    @Test("tap: if firstTap is column, if this tap is any free cell, move to first empty free cell")
    func tapSecondFreecell() async {
        do {
            // cannot move from a freecell to a freecell
            subject.state.firstTap = Tap(category: .freeCell, index: 0)
            subject.state.layout.freeCells[0].card = .init(rank: .two, suit: .clubs)
            subject.state.layout.columns[0].cards = [.init(rank: .six, suit: .hearts)]
            let oldLayout = subject.state.layout
    ...
    @Test("tap: if firstTap is freeCell, if this tap is column, move it if movable")
    func tapSecondColumnFromFreeCell() async {
        ...
        do {
            subject.state.firstTap = Tap(category: .freeCell, index: 0)
            subject.state.layout.freeCells[0].card = .init(rank: .five, suit: .clubs)
            subject.state.layout.columns[0].cards = [.init(rank: .six, suit: .hearts)]
            await subject.receive(.tapped(.init(category: .column, index: 0)))
    ...
    @Test("tap: if firstTap is column, if this tap is column, move maximum movable")
    func tapSecondColumnFromColumn() async {
        do {
            ...
            subject.state.firstTap = Tap(category: .column, index: 1)
            let oldLayout = subject.state.layout
            await subject.receive(.tapped(.init(category: .column, index: 0)))
            #expect(subject.state.layout == oldLayout)
            #expect(subject.state.firstTap == nil)
